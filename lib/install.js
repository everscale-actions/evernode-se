...
async function getReleaseAssetDownloadUrls(binariesRelease) {
  try {
    const binDownloadUrl = getDownloadUrl(binariesRelease.assets, (asset) => asset.name === `${releasePrefix}-${platform}-${binariesRelease.tag_name}.tar.gz`);
    const hashDownloadUrl = getDownloadUrl(binariesRelease.assets, (asset) => asset.name === `${releasePrefix}-${platform}-${binariesRelease.tag_name}.tar.gz.sha256`);
