...
async function getReleaseAssetDownloadUrls(binariesRelease) {
  try {
    const binDownloadUrl = getDownloadUrl(binariesRelease.assets, (asset) => asset.name === `${releasePrefix}-${platform}.tar.gz`);
    const hashDownloadUrl = getDownloadUrl(binariesRelease.assets, (asset) => asset.name === `${releasePrefix}-${platform}.tar.gz.sha256`);
    return { binaries: binDownloadUrl, hash: hashDownloadUrl };
  } catch (ex) {
    if (ex.message === 'Release is not found') {
      throw new ReleaseNotFound(binariesRelease.tag_name);
    }
    throw ex;
  }
}
...
async function install() {
  ...
  const cacheReleaseFolder = path.join(global.cachePath, binariesRelease.tag_name);
