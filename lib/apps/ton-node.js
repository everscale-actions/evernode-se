...
function transformMainConfig() {
  ...
  cfg.api.port = global.nodeSeRequestsPort;
  ...
  // copy zerostate
  cpSync(join(config.appPath, 'workchains'), join(config.dataPath, 'workchains'), { force: false, recursive: true });
}
...
function transformLogConfig() {
  ...
  cfg.appenders.tvm_logfile.path = join(config.logsPath, 'tvm.log');
